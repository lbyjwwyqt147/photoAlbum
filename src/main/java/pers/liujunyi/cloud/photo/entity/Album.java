 ...
 * 其他说明: @Document(indexName = "photo_manage", type = "album", shards = 1, replicas = 0) 默认情况下添加@Document注解会对实体中的所有属性建立索引  indexName elasticsearch 的索引名称 可以理解为数据库名 必须为小写 不然会报org.elasticsearch.indices.InvalidIndexNameException异常  type类型 可以理解为表名
...
@Document(indexName = "photo_manage", type = "album", shards = 1, replicas = 0)
@DynamicInsert
@DynamicUpdate
